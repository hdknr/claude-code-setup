...
- **`autoUpdate` の設定手段は複数ある**（`/plugin` の画面・`known_marketplaces.json` の
  ...
  **ただし確かめたのは「取り直しに行く」ところまで**——新しいコミットを引くところ・
  起動時に発火するところ・走る間隔は測っていない。
- **リポジトリ側からできるのは「気づける材料を置くこと」まで**——**ただし `autoUpdate` は
  例外かもしれない**。`extraKnownMarketplaces` のスキーマは「**リポジトリの
  `.claude/settings.json` で使うのが典型**」と書いており、**宣言できる可能性がある**。
  **ただし実体には `project` / `local` を除外する経路も見える。**
  **試していないので、できるともできないとも書かない**（一度「できない」と書いて撤回した——
  根拠にした文字列が別の場面のものだった）。更新の実行と再起動はクライアント側で確定。
