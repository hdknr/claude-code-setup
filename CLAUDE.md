...
- **`autoUpdate` の設定手段は複数ある**（`/plugin` の画面・`known_marketplaces.json` の
  ...
  **ただし確かめたのは「取り直しに行く」ところまで**——新しいコミットを引くところ・
  ...
  `claude plugin list` でも `-p` の起動でも**再現しなかった**（`--debug-file` のログ 38 行に
  更新関連は 0 行）。**「必ず起きる」ではなく「起きることがある」と書く。**
- **リポジトリ側からできるのは「気づける材料を置くこと」まで**——**`autoUpdate` は例外で、
  ...
  **条件は workspace が信頼済みであること**——未信頼だと `this workspace has not been trusted`
  として**設定ファイルごと落ちる**（同じ実行に `permissions.allow` を 1 件置いて対照を取った）。
  **書き込み先は利用者全体の `known_marketplaces.json` で、宣言を消しても残る**——
  つまり**リポジトリが利用者のクライアント状態を恒久的に書き換えられる**。
  **未登録の名前はセッション限り**で 3 ファイルのどこにも残らない。
  **`claude plugin` サブコマンドは宣言を見ない。** 更新の実行と再起動はクライアント側で確定。
