...
- **更新は「カタログを取り直す → プラグインを上げる」の 2 つを走らせる**
  （`claude plugin marketplace update` / `claude plugin update`。CLI から非対話で走る）。
  **判定と更新は分けられない**——最新でなければその場で更新される。
  **取り直しが必須かどうかは未検証**で、**むしろ CLI には更新前に取り直そうとする形跡がある**
  （`Failed to refresh marketplace '...' before update; using cached data:`）。
  決着には**クローンを凍結状態に戻して `plugin update` だけ走らせる**必要がある。
  それまでは**2 つとも走らせる**（取り直しは安全）。
  なお `--available` に **`dev-loop` は現れない**（理由は不明。「入っているから」ではない——
  入っている 12 件のうち 7 件は出る）ので、少なくとも `dev-loop` については比較に使えない。
- **リポジトリ側からできるのは「気づける材料を置くこと」まで。** 更新の実行・`autoUpdate` の
  有効化・再起動は**すべてクライアント側**で、リポジトリからは設定できない。
