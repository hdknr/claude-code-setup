...
Claude Code の機能は、**スキル**と**コマンド**で拡張できます。どちらも `/` に続けて
名前を入力すると実行できます。

| | 中身 | 呼び出し |
| --- | --- | --- |
| **スキル** | `SKILL.md` に手順を書いたもの。必要になった時点で本文が読み込まれる | `/名前` |
| **コマンド** | 定型の指示を 1 ファイルにまとめたもの | `/名前` |

**使う側の違いはほとんどありません。** ただしプラグインが配るときは種別が分かれるので、
下の一覧では「種別」列で示しています。
...
各プラグインの機能と入れ方は [プラグイン一覧](plugins/index.md) にまとめています
（`dev-loop` の手順の中身と前提は、そこからリンクしている README とスキル本文にあります）。
