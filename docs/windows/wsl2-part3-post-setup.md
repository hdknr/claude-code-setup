...
!!! info "ネイティブ Windows をご利用の方"
    PowerShell ベースの手順は [Windows / Part 3](part3-post-setup.md) を参照してください。
...
!!! warning "Windows 側（/mnt/c/...）には置かない"
    `/mnt/c/Users/...` の下にプロジェクトを置くと I/O が遅くなり、Claude Code の動作も重くなります。必ず **Ubuntu のホーム（`~/Projects/`）配下**に作成してください。
