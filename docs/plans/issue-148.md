...
- **`prepare-worktree.sh` が `git fetch` を打つかどうか。**
  打たないと `origin/<既定>` 自体が古い。打つと**ネットワーク操作**になり、
  オフラインで止まってはならない。**最善努力で打ち、失敗したら黙らない**方針で作るが、
  **「fetch すべきでない」という判断もありうる**ので関門に問う。
- **既定ブランチの解決順序。** `refs/remotes/origin/HEAD` が無いクローンは珍しくない。
  **推測（`origin/main` → `origin/master`）まで落ちるか、そこで止まるか。**
...
- 周の在り処: 上記「0. 周の在り処」
- 交絡を潰した手順: **未着手**（V1・V2 は実験なので、**陽性対照を取る**と上に書いた）
- Verifier 1 パス目: **未着手**
