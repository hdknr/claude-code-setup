  ...
  宣言し PR コメントに残す、のいずれかを明示的に選びます。
    - **検証・読解の委譲先は、`Bash` は持つが直接の書き込みツールは持たない `subagent_type` を
      ...
      **「`Write`／`Edit` が無い」＝「直接の書き込みツールが無い」ではありません**——除外リスト
      方式の agent は MCP の書き込みツールを継承しうるためです。**確実に満たすには、持たせる
      ツールを列挙した agent 定義を自分で置きます**（`.claude/agents/<name>.md` の `tools:` に
      `Read, Grep, Glob, Bash` を列挙する等）。組み込み agent を使うなら**指定前に実際の
      ツール一覧を確認します**。
