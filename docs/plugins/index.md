...
!!! danger "`claude plugin list` は「読み取りだけ」ではありません"
    ...
    **ただし 2026-09-14 には観測できませんでした。** `claude plugin list` を
    `--debug-file` つきで動かしても、ログ 38 行に更新に関する行は **1 行も出ず**、
    `lastUpdated` も動きませんでした（`-p` でのセッション起動でも動かず）。

    **これは「発火しない」の証拠ではありません。** そのログが覆うのは
    **プロセスが生きていた 0.13 秒**だけで、遅れて走る分は見ていません
    （実体には最大 10 分ほどの待ちを入れる形が読めます）。条件も揃っていません——
    #80 の観測は **`list` と `--help` の両方**を、**フラグを立てた直後**に、
    **2 日止まっていたマーケットプレイス**に対して動かしたときのものです。
...
| `SKILL.md` にバナーを刻む | **掃引を起こすこと**——リポジトリからできるのは掃引に**加わる**ことまでで、**起こす**のはクライアント側です（下記） |
...
1. **Issue 選択** → 2. **文脈収集** → 3. **計画** → 4. **実装** →
5. **検証（停止条件）** → 6. **レビュー → PR** → 7. **本番反映** → 8. **経験の還元**

**各手順の必須要件はここに書きません。** 受入基準の展開のしかた、別モデルの検証エージェント、
BLOCKED の報告、実験の交絡の潰し方、2 つの関門に 2 パス当てること——どれも
[プラグインの README](https://github.com/hdknr/claude-code-setup/blob/main/plugins/dev-loop/README.md)
と[スキル本文](https://github.com/hdknr/claude-code-setup/blob/main/plugins/dev-loop/skills/dev-loop/SKILL.md)
に**そこだけ**置いてあります。

**ここに写すと、片方だけが古くなります。** 実際に #75 の周で、手順 5 に足した参照が
README には入って**このページには入りませんでした**（気づいたのは 3 日後、
別の周のレビューでです——**関門が拾わなければ、そのままでした**）。
規範を足すのは `SKILL.md` で、README はその要約、このページは**入口**です。
...
!!! note "「小さいから」では縮退しません"
    ...
    [dev-loop の設計 §4.1](dev-loop-design.md#degradation)、BLOCKED と「未証明」の違いは
    [§2.3](dev-loop-design.md#blocked)、実験の交絡は
    [§2.4](dev-loop-design.md#confounded) を参照してください。
