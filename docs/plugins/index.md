...
5. **検証（停止条件）** — 実機で目視確認 ＋ **別モデルの検証エージェント**で受入条件の反証を探す。
   ...
   ツール構成で表明する。指定前に実際のツール一覧を確認する）。回せないときは**黙って飛ばさず、着手前に申告して指示を仰ぐ**
...
- **別モデルの検証エージェント**（`Agent` ツール等）— 手順 5 で**必須**です。レビューとは**相互に
  ...
  **黙って飛ばさず、着手前に申告して指示を仰ぎます**——(1) 使用許可を求める / (2) `/code-review` に
  ...
  宣言し PR コメントに残す、のいずれかを明示的に選びます。
    - **検証・読解の委譲先は書き込み手段を持たない `subagent_type` を既定にします**（`Bash` は持つ）。
      「ファイルを変更するな」と書いて汎用 agent に渡す形は fallback であって既定ではありません。
      **実装の委譲先は対象外**です（書けなければ仕事になりません）。
      **「`Write`／`Edit` が無い」＝「書けない」ではありません**——除外リスト方式の agent は MCP の
      書き込みツールを継承しうるので、**指定前に実際のツール一覧を確認します**。
