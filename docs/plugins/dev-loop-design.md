...
!!! note "表記について"
    プラグインとして入れた場合の実際の呼び出しは **`/dev-loop:dev-loop`** です（プラグイン提供のスキルは常にプラグイン名で名前空間化されます）。
    この文書では読みやすさのため、スキルの通称として `/dev-loop` と表記します。
    導入手順は [プラグイン一覧](index.md#dev-loop) を参照してください。
...
| 選択肢 | 得られるもの | 注意 |
| --- | --- | --- |
| **使用許可を求める**（第一候補） | 敵対的な別モデル視点（本来の牽制） | 別モデル＝**下位ティア**なので、本実装より軽いコストで済む旨を添える |
