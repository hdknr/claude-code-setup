...
| 選択肢 | 得られるもの | 注意 |
| --- | --- | --- |
| **使用許可を求める** | 敵対的な別モデル視点（本来の牽制） | 別モデル＝**下位ティア**なので、本実装より軽いコストで済む旨を添える |
| **`/code-review` で代替** | `Agent` に依らないレビュー | 手順 6 と同じ道具。**別モデル視点は得られない**ことを伝える |
| **理由付きで省略を宣言** | 何を検証していないかの記録 | Issue／PR コメントに残し、後から追える状態にする |

!!! warning "2 つの関門は独立。片方が落ちてももう片方は残る"
    手順 6 の `/code-review` は **Verifier の有無に関わらず独立に必須**。`Agent` を使わないので、
    `Agent` が禁止されたセッションでも回せる。手順 5 が落ちたときに手順 6 まで一緒に落ちるのが
    典型的な事故の形。
...
（[§2.1](#gate-conflict)）。黙って 5.5 へ進んではならない。
