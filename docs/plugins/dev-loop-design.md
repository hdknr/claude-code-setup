...
!!! danger "`/code-review` は「`Agent` フリーの逃げ道」ではない"
    ...
    コマンドは Haiku 1 体で対象を絞り、**Sonnet 5 体を並列**で走らせ、指摘ごとに Haiku で
    確信度を採点する）。つまり **`Agent` が禁止された周は、手順 5 と手順 6 が同時に落ちうる**。

    手順 6 の `/code-review` は **Verifier の有無に関わらず独立に必須**——手順 5 が落ちたときに
    手順 6 まで一緒に落ちるのが典型的な事故の形だからだ。ただし「片方は必ず残る」とは考えられない。
    **両方が同じ禁止で落ちうるからこそ、唯一の防波堤は申告になる。**
...
6. **レビュー** — `/code-review` で差分をレビューし、指摘を反映して PR を作成。**`/code-review` は
   手順 5 の Verifier の有無に関わらず独立に必須**。指摘を直したら、その修正自体を手順 5 の verify に
