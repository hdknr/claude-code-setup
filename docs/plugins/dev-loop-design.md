...
6. **レビュー** — `/code-review` で差分をレビューし、指摘を反映して PR を作成。**`/code-review` は
   ...
   一度も通っていない）。**この 2 パスは差分の大小に依らず必要**。**worktree 上にいることが PR 作成の
...
```bash
# 受入基準を渡し、問いを局面に絞らない。素の /code-review では差分の局所読解に縮退する
/code-review 計画は docs/plans/issue-999.md。不変条件 1「is_new は外部から書き換えられない」を
破りうる経路（保存・一括更新・REST・管理画面・インポート）を全部挙げて、それぞれ守られているか
確認してほしい

# 指摘を直したら「verify に当て直す」だけでは足りない。増えたコードはレビューを通っていない
/code-review                        # 指摘対応で書いたコードを、もう一度レビューに掛ける
