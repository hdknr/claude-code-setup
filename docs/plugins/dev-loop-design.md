...
!!! warning "生成物があるなら、その鮮度を受入基準に入れる"
...
6. **レビュー** — `/code-review` で差分をレビューし、指摘を反映して PR を作成。**`/code-review` は
   ...
   前提条件**で、そうでなければ worktree へ移送してから作る。結果は PR コメントに残す。
...
受入条件の充足は [§2](#verify) のとおり**別モデルの Verifier** に判定させる。差分が大きいときは
...
`/code-review` は **5.4 の Verifier を回せたかどうかに関わらず必須**。指摘を直したら、その修正を
**母集団に当て直してから** PR にする。

```bash
/code-review                        # 差分をレビューし指摘を反映（Verifier の有無に関わらず必須）
