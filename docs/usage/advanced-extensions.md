...
!!! tip "このリポジトリが配布するプラグイン"
    本リポジトリ自体もプラグイン・マーケットプレイスとして `workspace-setup` / `cmux` / `dev-loop` を配布しています。各プラグインの機能と入れ方は [プラグイン一覧](../plugins/index.md) にあります
    （`dev-loop` の手順と規範は、そこからリンクしている README とスキル本文が正です）。
