...
- **同じファイルの中の自己矛盾**——同じ語句に違う数が付いていたら落とす。
  **ただし見るのは、助詞（`が` ／ `は`）を伴う件数表現だけである**（下記）。
...
- **助詞を伴わない件数表現**（#150）——`COUNTED` が **`が` か `は` を要求する**ので、
  **`次の 6 つを書く` のような形はそもそも収集されない。**
  **同じファイルに `次の 6 つ` と `次の 5 つ` が同居していても緑である。**
  **一度これを「守る」側だけに書いて、実装より広い約束になっていた**
  ——**#145 の周で実際に当てて、緑のまま通ることが確かめられている。**
  **助詞を任意にする案は測ってから捨てた**——**このリポジトリの地の文では
  `次の…つ` のような指示語つきの件数が別々の列挙を指して当たり前に並ぶので、
  どの広げ方でも正しい記述どうしの衝突が支配する。**
  **母集団・広げ方ごとの発火数・誤検出の中身は
  [#150](https://github.com/hdknr/claude-code-setup/issues/150) と
  `docs/plans/issue-151.md` の「測定」を正とする。**
