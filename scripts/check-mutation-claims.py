...
- **走るたびに結果が揺れる `red` では「主張が偽」と断定しない**（#156）。
  **`緑のまま` に倒す直前に、次の 4 つを*この順に*確かめる**
  （**どれか 1 つでも欠けると、実際に退行が起きた**——経緯は
  `docs/plans/issue-156.md` §5 を正とする）:

  1. **変異を戻して 2 回続けて走らせ、その 2 回が揃うか**（揺れていれば倒す）。
     **測るのは隣り合う 2 回で、キャッシュした 1 度目とは比べない**
     ——**`red` の「初回だけの副作用」を揺れと読むと、本物の「主張が偽」まで倒す。**
  2. **その 2 回が緑か**（`rc == 0`）。**揃っていても、陽性対照として死んでいれば
     「主張が偽」とは言えない。**
  3. **もう一度変異を当てて走らせ、その結果を 1 の結果と比べる**
     ——**変異を当てた最初の回と比べてはならない。** **あれは 1 より*前*に測っており、
     2 段階の準備をする `red` では準備の段差を「変異のせい」と読む。**
  4. **その回も緑か**（`rc == 0`）。**非ゼロなら「rc=0 のまま」と言ってはならない。**

  **SKIP かどうかは見ていない**——**判定して見逃した `red` でも、出力に時刻や件数が
  混じれば同じ経路で倒れる**（**下の「判定したかどうかは見ていない」と同じことを、
  こちら側からも言っている**）。
  **これが無いと、0 で終わる SKIP のうち出力が変わるものが素通りする**
  ——**上の「出力が変わらない」は、たまたま一致する場合しか捕まえない。**
  **どの回でも走らなかった場合は、それと分かる別の理由で倒す**（打ち切り・コマンド不在）。
...
def judge(claim: Claim, box: pathlib.Path, baselines: dict) -> tuple[str, str]:
    ...
    # **当て直した回が非ゼロなら、「rc=0 のまま」と言ってはならない。**
    # **測った証拠を捨てて、literally 偽の文を出すことになる**——**この歯止めが
    # 存在する当の理由**である（3 パス目の `/code-review` が実測で反証した）。
    # **`first[0] != 0` は k+1・k+2 で現れる腐りしか捕まえない**ので、
    # **もっと後から落ち始める `red`** はここまで来る。
    if again[0] != 0:
        return NOT_APPLIED, f"変異を当てた回の rc が再現しない（{rc} → {again[0]}）"
    if again == first:
