...
def judge(claim: Claim, box: pathlib.Path, baselines: dict) -> tuple[str, str]:
    ...
    # **走らなかったことを「揺れている」と言わない。** `run()` は打ち切り・コマンド不在で
    # `None` を返すので、**下の比較はそれを「違った」として飲み込む**——
    # **判定は同じでも理由が偽になる**（`red` が揺れたのではなく、走らなかった）。
    # **上の `rc is None` が同じ状況を別扱いしているのに、ここには無かった。**
    if first[0] is None or second[0] is None:
        why = first[1] if first[0] is None else second[1]
        return NOT_APPLIED, f"陽性対照の当て直しが走らなかった（{why}）"
    if first != second:
        return NOT_APPLIED, ("変異を戻して 2 度走らせても同じ結果にならない"
                             "（この `red` では出力が変異の有無を判別できない）")
    # **ここまで来れば `red` は隣り合う 2 回で揺れていない。**
    # **変異を当てた結果がそれと違うなら、違いは変異に帰せる。**
    if (rc, out) == first:
