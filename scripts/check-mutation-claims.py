...
- **中断されたときも畳む**——**`Exception` ではなく `BaseException` で受ける**（#157）。
  **`KeyboardInterrupt` は `Exception` では捕まらない**のに、**歯止めが残骸を残すのは
  まさに中断されたとき**である。
- **出力が実行ごとに変わる `red` では「主張が偽」と断定しない**（#156）。
  **`緑のまま` に倒す直前に陽性対照をもう 1 度走らせ、同じ結果でなければ
  `当てられなかった` に倒す**——**出力に時刻・件数・乱数が混じる `red` が SKIP すると、
  変異と無関係に出力が変わる**ので、**上の「出力が変わらない」だけでは素通りする。**
...
- **「決定的である」ことは証明できない**（#156）。**2 回とも同じだった、としか言えない。**
  **低い確率で出力が変わる `red` は、いまでも `緑のまま` に落ちうる。**
  **逆向きにも倒れる**——**木の状態が戻らなかった周は、変異と無関係に 2 度目が変わるので
