...
| 守らないもの | なぜ |
| --- | --- |
| **マーカーを持たない規範の複製** | 数えているのは `（必須）` という文字列だけ。`**太字**` で書いた規範は通る。**#94 で `README.md` から外した規範はほとんどがこの形で、この検査では捕まらなかった**（行数は書かない——測り直すと黙って古くなる。設計 §8.1 を見ること） |
...
# インラインコード。**マーカーを含むものだけを狙わない。**
#
# 最初はマーカーを含む囲みだけを拾う形（`` `[^`]*（必須）[^`]*` ``）にしていたが、
# **2 つの囲みの間に素のマーカーがある行を見逃した**——`` `A` は（必須）で `B` `` のような、
# このリポジトリでは**ごく普通の書き方**である。最初の囲みの閉じと次の囲みの開きが
# 1 つの囲みとして一致し、**間の素のマーカーごと消えていた**（#94 のレビューが再現例つきで指摘）。
#
