...
# ## 交絡について
#
# **揃えられるものは揃えてあるが、「振った次元は OS だけ」ではない。**
#
# | 次元 | 揃え方 |
# | --- | --- |
...
#
# **揃っていないものが 3 つ残る**（一度ここに「1 つ」と書いて、手順 5 の Verifier に
# 2 度反証された。**訂正が一部の場所にしか届かない**のがこの型である）:
#
# 1. **git の版**（macOS 2.54.0 / 像 2.39.5）。ガードは harness の機能で git の機能では
#    ないが、揃っていないことは記録する。
# 2. **資格情報の運び方**——**コンテナ分岐だけが `CLAUDE_CODE_OAUTH_TOKEN` を注入し、
#    `--host` 分岐は何も注入しない**（下の実装のとおり）。**口座は同じでも運び方が違う。**
#    一度ここに「どちらも同じサブスクリプションの資格情報」と書いていたが、
#    **それは同じファイルの実装と食い違っていた。**
# 3. **利用者の個人設定と hooks**——`--host` は実ホームの `settings.json`
#    （権限モード・hooks・有効なプラグイン）の下で走るが、**コンテナは素の
#    `npm install` で個人設定を 1 つも持たない**。
#
# **3 つとも、結論に添えること。**
#
# ## 資格情報の運び方（コンテナ側）
