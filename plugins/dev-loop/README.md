...
5. **検証（停止条件）** — 実機で目視確認 ＋ **別モデルの検証エージェント**で受入条件の反証を探す。
   検証の委譲先は**書き込み手段を持たない `subagent_type` が既定**（禁止は散文ではなく
   ツール構成で表明する。指定前に実際のツール一覧を確認する）。
...
- **別モデルの検証エージェント**（`Agent` ツール等）— 手順 5 で**必須**。レビューとは**相互に
  ...
  **黙って飛ばさず、着手前に申告して指示を仰ぐ**——(1) 使用許可を求める / (2) `/code-review` に
  ...
  宣言し PR コメントに残す、のいずれかを明示的に選ぶ。
    - **検証・読解の委譲先は書き込み手段を持たない `subagent_type` を既定にする**（`Bash` は持つ）。
      「ファイルを変更するな」と書いて汎用 agent に渡す形は fallback であって既定ではない。
      **実装の委譲先は対象外**（書けなければ仕事にならない）。
      **「`Write`／`Edit` が無い」＝「書けない」ではない**——除外リスト方式の agent は MCP の
      書き込みツールを継承しうるので、**指定前に実際のツール一覧を確認する**。
      `Bash` があればリダイレクトでも書けるため、**実環境を壊しうる検証は渡す側で
      サンドボックス化する**（テストスクリプト側で `HOME` を差し替える等）。
