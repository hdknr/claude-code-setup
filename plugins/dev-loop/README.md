  ...
  宣言し PR コメントに残す、のいずれかを明示的に選ぶ。
    - **検証・読解の委譲先は、`Bash` は持つが直接の書き込みツールは持たない `subagent_type` を
      ...
      **「`Write`／`Edit` が無い」＝「直接の書き込みツールが無い」ではない**——除外リスト方式の
      agent は MCP の書き込みツールを継承しうる。**確実に満たすには、持たせるツールを列挙した
      agent 定義を自分で置く**（`.claude/agents/<name>.md` の `tools:` に `Read, Grep, Glob, Bash`
      を列挙する等）。組み込み agent を使うなら**指定前に実際のツール一覧を確認する**。
