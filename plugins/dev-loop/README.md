...
3. **計画** — 変更範囲を切り分け、設定／フラグで済むかを先に判断。**計画と verify の受入基準を
   ...
   **「不変条件 × それを破りうる経路」に展開**する（**経路は差分ではなくアプリの入口一覧から
   ...
   受入基準に入れる（該当が無ければ「生成物なし」と明記する）。あわせて **5 番目の項目として
   「関門の進捗（再開点）」**を書く——**どの関門をどのパスまで通したか**（**結果まで**。
   「起動済み」は進捗ではない）・**手順 5 の反証と手順 4 への差し戻し**・**交絡を潰した手順**。
   ここには**実績だけ**を書く（規定の引用を混ぜると、再開した側が実績と読み違える）
4. **実装** — **まず worktree を開始**してから、対象リポジトリの `CLAUDE.md` のルールに従って変更。
   **ファイルを変更する周では worktree は必須**で、その周の成果は必ず PR になる
   （例外は調査だけで終わる周）。実装を終えたら**割り目**を置く——再開点を書き、
   **`/clear` → `/dev-loop <n>` で手順 5 から再開できる**ことを人間に伝える
   （必須なのは**再開点を書くことと提案すること**で、割ること自体ではない。
   モデルは自分の文脈を消せない）。**関門が動いている最中に割り目を置かない**
5. **検証（停止条件）** — 実機で目視確認 ＋ **別モデルの検証エージェント**で受入条件の反証を探す。
   ...
   **検証の生出力は親の文脈に積まない**——ファイルに落として集計行だけ読む。
   **ただし「見ない」ではない**: **失敗の中身・交絡の判定・観測窓**に要る範囲は読む
   （全文を読む役は別コンテキストの Verifier）
6. **レビュー → PR** — **`/code-review` は手順 5 の有無に関わらず必須**。**受入基準は Verifier と
   ...
   **2 パス目の前が 2 つ目の割り目**（再開点を更新してから分割を提案する）。
   **2 パス目が読む対象の差分は指摘対応分に絞ってよい**——**絞れないものの一覧は
   `SKILL.md` の手順 6 を正とする**（ここに列挙しない。列挙を 2 箇所に置くと片方だけが古くなる）。
  ...
  宣言し PR コメントに残す、のいずれかを明示的に選ぶ。
    - **検証・読解の委譲先は、`Bash` は持つが直接の書き込みツールは持たない `subagent_type` を
      ...
      **「`Write`／`Edit` が無い」＝「直接の書き込みツールが無い」ではない**——除外リスト方式の
      agent は MCP の書き込みツールを継承しうる。
      **このプラグインは条件を満たす定義を同梱している**ので、`subagent_type` に
      **`dev-loop:dev-loop-verifier`** を指定すればよい（手で書く必要はない）。
      **名前空間を落として `dev-loop-verifier` と書くと、解決に失敗して汎用 agent に静かに
      フォールバックする**ので注意する。素のスキルとして使う場合は、同じ形の定義を
      `~/.claude/agents/<name>.md` に置く（`tools:` に `Read, Grep, Glob, Bash` を列挙。
      `model:` は書かず、周ごとに `Agent` の `model` 引数で別ティアを指定する）。
